...
const Slider = forwardRef(({ dataState, navigate, LeftOutlined, PlayCircleFilled, RightOutlined }, ref) => {
    ...
    // GIẢI PHÁP CLS: Nhân bản mảng dữ liệu để cuộn vô tận, không can thiệp DOM thật của React (GIỮ NGUYÊN LOGIC)
    const infiniteMovies = useMemo(() => {
        if (isLoading) return [];
        return [...moviesList, ...moviesList, ...moviesList];
    }, [moviesList, isLoading]);

    // Kích hoạt quản lý vòng đời tự động cuộn (GIỮ NGUYÊN LOGIC)
    useEffect(() => {
        if (isLoading) return;
    ...
    return (
        <div className='relative group border-b border-gray-900 pb-4 min-h-[285px] sm:min-h-[345px] md:min-h-[375px]'>
            {/* Nút bấm slide sang trái (Đồng bộ khoảng cách cuộn chuẩn hệ thống 1.5rem gap) */}
            <button
                onClick={() => {
            ...
            {/* Khung chứa Slider (Khớp thuộc tính (.halim-trending-track) hệ thống bạn gửi) */}
            <div
                ref={sliderRef}
                style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
                className='halim-trending-track flex gap-6 overflow-x-auto flex-nowrap scroll-smooth pb-6 pt-4 px-4 w-full select-none will-change-[transform,scroll-position]'
            >
                {isLoading ? (
                    ...
                    infiniteMovies.map((movie, index) => {
                        ...
                        const rankNumber = (index % moviesList.length) + 1;

                        // PHÂN TÁCH CLASS THEO ĐÚNG SOURCE GỐC: Vị trí chẵn nhận even, vị trí lẻ nhận odd phẳng lặng
                        ...
                        return (
                            <div
                                key={`slider-item-${movie?.id || index}-${index}`}
                            ...
                            >
                                {/* KHU VỰC KHUNG CHỨA BANNER (Đồng bộ hiệu ứng scale, lớp loaded xóa sau :after) */}
                                <div className={`halim-trending-poster-container halim-trending-poster-loaded ${clipPathClass} w-full transition-transform duration-300 ease-out z-1`}>
                                    ...
                                    {/* 1. ẢNH BANNER PHIM (Z-index: 1 thẳng đứng hoàn toàn, không lật) */}
                                    <img
                                        src={movie?.posterUrl}
                                    ...
                                    {/* 2. LỚP MASK PHỦ MỜ (Đúng định thức .halim-trending-poster-mask của hệ thống) */}
                                    <div className='halim-trending-poster-mask' />
                                    
                                    {/* Nút Play hiển thị ở tâm khi hover */}
            ...
            {/* Nút bấm slide sang phải (Đồng bộ khoảng cách cuộn chuẩn hệ thống 1.5rem gap) */}
            <button
                onClick={() => {
                    if (sliderRef.current) {
                        const firstItem = sliderRef.current.firstElementChild;
